# My Modifications to The Scraper

Rather than scrape a single headline from the Daily Pennsylvanian homepage, I wanted to scrape multiple headlines. I decided to scrape all the headlines under the "Featured" section in the homepage. First, I went on the homepage and inspected the featured section. I found out that all of the featured articles are contained in a div element with the class attribute of "__flexy-box", so I used the find method with the input "div" and "class_="__flexy-box"" to get the container of all the featured articles. Then, I needed a way to access each individual article in this container, so I looked up the documenation for bs4, and read through doc until I found something I could use. That's where I found ".children", which returns an iterator of the children elements of the parent element, which in my case, the parent is an element containing featured articles and the children is each individual featured article. From that, I used a for loop to iterate through each featured article. I tested my code by printing the results to the terminal, and I initially discovered that some of the children elements from ".children" are just whitespace, so I checked if the child element is just whitespace before proceeding. If the child element is not whitespace, then I get its headline and format it accordingly before adding it to the string of all headlines, which I return after all the iterations.
